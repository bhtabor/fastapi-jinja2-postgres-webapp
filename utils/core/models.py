...
class PasswordResetToken(SQLModel, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    account_id: int | None = Field(foreign_key="private.account.id")
...
class AccountToken(SQLModel, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    account_id: int | None = Field(foreign_key="private.account.id", index=True)
    token: str = Field(index=True)
    context: str
    sent_to: str | None = Field(default=None)
    inserted_at: datetime = Field(default_factory=utc_now)

    account: Mapped[Account | None] = Relationship(back_populates="tokens")
...
class User(UserBase, table=True):
    ...
    account_id: int | None = Field(foreign_key="private.account.id", unique=True)
